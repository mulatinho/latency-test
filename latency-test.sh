...
if [ $KAFKAENABLE -eq 1 ] ; then
        ...
        echo "HELLOWORLD" | kafka-console-producer.sh --broker-list $KAFKAHOST:$KAFKAPORT --topic $KAFKATOPIC --property group.id=latency
        ...
        kafka-topics.sh --delete --topic latency_topic_test --zookeeper $ZOOKEEPERHOST:$ZOOKEEPERPORT
fi
